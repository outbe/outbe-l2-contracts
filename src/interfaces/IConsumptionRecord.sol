...
interface IConsumptionRecord is ISoulBoundToken {
    ...
    /// @param tokenId The hash of the consumption record (must be non-zero)
    ...
    function submit(uint256 tokenId, address owner, string[] memory keys, bytes32[] memory values) external;
